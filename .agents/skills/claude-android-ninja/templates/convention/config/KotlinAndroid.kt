...
internal fun Project.configureKotlinAndroid(commonExtension: CommonExtension) {
    commonExtension.apply {
        compileSdk { version = release(libs.findVersion("compileSdk").get().toString().toInt()) }

        defaultConfig.apply { minSdk = libs.findVersion("minSdk").get().toString().toInt() }
...
/**
 * Configure Kotlin compiler options via KotlinCompile tasks. Works with AGP 9+ built-in Kotlin
 * where KotlinAndroidProjectExtension is not registered.
 */
...
private inline fun <reified T : org.jetbrains.kotlin.gradle.dsl.KotlinBaseExtension> Project
    .configureKotlin() =
    configure<T> {
        val warningsAsErrors =
            providers.gradleProperty("warningsAsErrors").map { it.toBoolean() }.orElse(false)
