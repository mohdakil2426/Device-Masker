...
- [ ] 1.6 Run `./gradlew :xposed:compileDebugKotlin :app:compileDebugKotlin :common:compileDebugKotlin` — final compile verification after all hooker rewrites
...
- [ ] 2.8 Verify: Build `./gradlew :xposed:assembleDebug` — should compile
...
- [ ] 3.2 Update `app/src/main/kotlin/.../data/XposedPrefs.kt`: Replace `Context.MODE_WORLD_READABLE` with `ModulePreferences.from(context, "device_masker_config")`. Add `init(context)` method.
...
- [ ] 3.4 Update `app/src/main/kotlin/.../DeviceMaskerApp.kt`: Call `XposedPrefs.init(this)` in `onCreate()`.
- [ ] 3.5 Update `app/src/main/kotlin/.../service/ConfigManager.kt` (app-side): Remove `syncToAidlService(json)` call. Config save path: write JSON file + ConfigSync → ModulePreferences only.
- [ ] 3.6 Verify: `ConfigSync.kt` uses `XposedPrefs.getPrefs(context).edit()` — no changes needed if key format unchanged
...
- [ ] 6.6 `getPhoneCount()` hook — **DEFERRED** (requires `DEVICE_PROFILE` sim count field — pending task 15)
- [ ] 6.8 `getNetworkType()` / `getDataNetworkType()` hooks — **DEFERRED** (requires `NetworkTypeMapper` — pending task 16)
- [ ] 6.12 Verify: Run `./gradlew :xposed:compileDebugKotlin`
...
- [ ] 7.5 Add `ConnectivityManager.getActiveNetworkInfo()` hook → `NetworkInfoHooker`. **(DEFERRED)**
- [ ] 7.6 Add `WifiManager.getConnectionInfo()` hook → `WifiConnectionInfoHooker`. **(DEFERRED)**
...
- [ ] 8.2 `getActiveSubscriptionInfoCount()` hook — **DEFERRED** (requires `simCount` field from task 15)
...
- [ ] 9.2 `xi.hookClassInitializer(buildClass)` — **DEFERRED** (complex; direct field mutation is sufficient)
- [ ] 9.6 `Build.VERSION.SECURITY_PATCH`, `Build.TIME`, `Build.ID`, `Build.SUPPORTED_ABIS` mutations — **DEFERRED** (requires task 15 preset enrichment)
...
- [ ] 10.4 Add `System.getProperty("http.agent")` hook → `SystemPropertyHooker`. **(DEFERRED)**
...
- [ ] 11.2 `hasSystemFeature` hooks (NFC, 5G) — **DEFERRED** (requires task 15 preset enrichment for `hasNfc`, `has5G`)
...
- [ ] 12.4 `Runtime.exec(String)` hook → `RuntimeExecHooker` — **DEFERRED**
- [ ] 12.5 `ActivityManager.getRunningServices()` hook → `RunningServicesHooker` — **DEFERRED**
...
- [ ] 14.1 Add `DEVICE_TAC_PREFIXES` map to value generators in `:common` (or `:xposed/utils`)
- [ ] 14.2 Implement `imeiForPreset(presetId)` function
- [ ] 14.3 Implement `generateImeiWithTac(tac)` private helper and `luhnCheckDigit(number)` utility
- [ ] 14.4 Update `GetImeiHooker` in DeviceHooker to call `imeiForPreset()`
- [ ] 14.5 Add unit tests for `imeiForPreset()`

## 15. DeviceProfilePreset Enrichment (Phase 13)

- [ ] 15.1 Add new fields to `DeviceProfilePreset` data class: `buildTime`, `securityPatch`, `buildId`, `incremental`, `supportedAbis`, `tacPrefixes`, `simCount`, `hasNfc`, `has5G`
- [ ] 15.2 Update Pixel 9 Pro preset with accurate enriched data
- [ ] 15.3 Update Samsung Galaxy S24 Ultra preset with accurate enriched data
- [ ] 15.4 Update remaining 8 presets with accurate enriched data
- [ ] 15.5 Add `findById(id)` companion object method if not already present
- [ ] 15.6 Verify: `:common` module compiles, existing tests pass

## 16. NetworkTypeMapper & Correlation Matrix (Phase 14)

- [ ] 16.1 Create `NetworkTypeMapper.kt`: Maps MCC/MNC prefixes to NETWORK_TYPE_LTE/NR
- [ ] 16.2 Verify `GetNetworkTypeHooker` in DeviceHooker correctly calls `NetworkTypeMapper.getForMccMnc()`
- [ ] 16.3 Verify correlation matrix (documentation/audit task)
- [ ] 16.4 Add unit test for `NetworkTypeMapper.getForMccMnc()`

## 17. AIDL Diagnostics Service — Option B (Phase 2/8 combined)

- [ ] 17.1 Rewrite `IDeviceMaskerService.aidl`: Reduce to 8 diagnostic-only methods, remove all config methods
- [ ] 17.2 Rewrite `xposed/service/DeviceMaskerService.kt`: Remove config state, keep diagnostics state
- [ ] 17.3 Delete `xposed/service/ConfigManager.kt` (xposed-side config — replaced by RemotePreferences)
- [ ] 17.4 Verify `xposed/service/ServiceBridge.kt` — no config references
...
- [ ] 17.6 Rewrite `app/service/ServiceClient.kt`: Remove config methods, keep diagnostics methods only
- [ ] 17.7 Update `DiagnosticsViewModel.kt`: Remove config-related service calls
- [ ] 17.8 Add `reportSpoofEvent(pkg, spoofType)` and `reportPackageHooked(pkg)` to `XposedEntry.kt`
- [ ] 17.9 Update all hooker `@AfterInvocation` methods: Add `XposedEntry.instance.reportSpoofEvent()` call
- [ ] 17.10 Verify: AIDL compiles, service compiles, ServiceClient compiles

## 18. ProGuard & Release Build (Phase 15)

- [ ] 18.1 Rewrite `xposed/consumer-rules.pro`: Add libxposed API 100 keep rules, remove YukiHookAPI rules
- [ ] 18.2 Update `app/proguard-rules.pro`: Remove YukiHookAPI rules, add libxposed-service rules
- [ ] 18.3 Update `common/consumer-rules.pro`: Update AIDL keep rules for new 8-method interface
- [ ] 18.4 Run `./gradlew assembleRelease` — verify R8 passes
- [ ] 18.5 Verify `mapping.txt`: All critical classes kept
- [ ] 18.6 Verify APK contents: `META-INF/xposed/java_init.list` exists, `assets/xposed_init` does NOT

## 19. Cleanup & Deletion

- [ ] 19.1 Delete `app/src/main/kotlin/.../hook/HookEntry.kt` (YukiHookAPI KSP entry)
- [ ] 19.2 Delete/archive `xposed/src/main/kotlin/.../utils/ClassCache.kt` if it exists
- [ ] 19.3 Delete `xposed/src/main/kotlin/.../utils/HookHelper.kt` (YukiHookAPI utilities — confirmed 0 callers)
- [ ] 19.4 Remove any remaining YukiHookAPI imports — `grep -rn "import com.highcapable"` must return 0 results **(Already verified clean: 0 results)**
- [ ] 19.5 Remove any remaining KavaRef imports — must return 0 results
- [ ] 19.6 Remove any remaining legacy Xposed API imports — must return 0 results
- [ ] 19.7 Clean up `_proposal_instructions.json`, `_design_instructions.json` temp files from openspec change directory
...
- [ ] 21.1 Unprotected hook callbacks: Verify 0 bare `after {` / `before {` patterns (API 100 uses `@XposedHooker` now)
- [ ] 21.2 Hardcoded pref keys: `"module_enabled"` etc. — must return 0 results
- [ ] 21.3 Insecure random: `Random()` in common/src — must return 0 results
- [ ] 21.4 Timber in xposed: `Timber.` — must return 0 results
- [ ] 21.5 Compose in xposed/common — must return 0 results
- [x] 21.6 YukiHookAPI remnants: `import com.highcapable` in xposed/src — **VERIFIED CLEAN (0 results)**
- [ ] 21.7 Legacy Xposed API remnants: `de.robv.android.xposed` — must return 0 results
...
- [ ] 22.2 Update `memory-bank/techContext.md`: Replace YukiHookAPI/KavaRef/API 82 references with libxposed API 100
- [ ] 22.3 Update `memory-bank/systemPatterns.md`: Replace hook loading order, BaseSpoofHooker pattern, hook safety pattern
- [ ] 22.4 Update `memory-bank/productContext.md`: Update data flow diagram
- [ ] 22.6 Update `GEMINI.md`: Update tech stack table, architecture diagram, hooker inventory
