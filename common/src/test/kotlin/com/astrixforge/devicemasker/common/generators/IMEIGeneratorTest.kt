...
class IMEIGeneratorTest {
    ...
    fun `generate returns only numeric characters`() {
        repeat(100) {
            ...
            assertTrue(
                imei.all { it.isDigit() },
                "IMEI should contain only digits, got: $imei"
            )
        }
    }
    ...
    fun `generate returns valid Luhn checksum`() {
        repeat(100) {
            ...
            assertTrue(
                ...
                "IMEI should pass Luhn validation, got: $imei"
            )
        }
    }
    ...
    fun `generated IMEIs are unique`() {
        ...
        assertTrue(
            imeis.size >= 990,
            "Expected nearly 1000 unique IMEIs, got ${imeis.size}"
        )
    }
