...
Select-String -Path 'app\build\outputs\mapping\release\mapping.txt' -Pattern 'DeviceMaskerService|XposedEntry|AntiDetect'
...
Get-ChildItem -Path 'app/src','xposed/src','common/src' -Recurse -Filter '*.kt' | Select-String 'BaseSpoofHooker'
...
1. Runs all 10 Xposed safety checks (0-result checks)
...
- **LibXposed**:
  ...
  - [Service Docs](https://libxposed.github.io/service/)
...
| **material-3-expressive** | Material 3 Expressive UI design, review, token specification, and motion.        | `.agents/skills/material-3-expressive/` |
...
_Last Updated: 2026-03-19 (libxposed API 101 full migration complete: module.prop, ProGuard, all hook docs, 10 hookers, RemotePreferences config path)_
