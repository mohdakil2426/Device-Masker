 ...
 *
 * Hooks SensorManager and Sensor classes to prevent device fingerprinting
 * via sensor enumeration.
 */
object SensorHooker : BaseSpoofHooker("SensorHooker") {
    ...
    private val ESSENTIAL_SENSOR_TYPES = setOf(
    ...
    private fun hookSensorManager() {
        sensorManagerClass?.apply {
            ...
            method { name = "getSensorList"; param(IntType) }.hook {
                after {
                    ...
                    if (sensorType == -1 && originalList.size > 10) {
                        ...
                        logDebug("Filtered sensor list: ${originalList.size} -> ${(result as List<*>).size}")
                    }
                }
            }

            // Hook getDefaultSensor(int type)
            method { name = "getDefaultSensor"; param(IntType) }.hook {
                after {
                    val sensorType = args(0).int()
    ...
    private fun hookSensorMetadata() {
        sensorClass?.apply {
            // Hook getVendor()
            method { name = "getVendor"; emptyParam() }.hook {
                after {
            ...
            method { name = "getVersion"; emptyParam() }.hook {
                after {
            ...
            method { name = "getName"; emptyParam() }.hook {
                after {
                    val originalName = result as? String ?: return@after
                    if (originalName.contains("Qualcomm") || 
