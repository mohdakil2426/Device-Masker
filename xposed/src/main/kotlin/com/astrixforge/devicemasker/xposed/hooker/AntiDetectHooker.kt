...
object AntiDetectHooker : YukiBaseHooker() {
    ...
    private val HIDDEN_CLASS_PATTERNS = listOf(
        "de.robv.android.xposed", "XposedBridge", "XposedHelpers",
        "XC_MethodHook", "XC_MethodReplacement",
        "io.github.lsposed", "LSPHooker", "LSPosed",
        "com.highcapable.yukihookapi", "YukiHookAPI", "YukiBaseHooker",
        "EdHooker", "EdXposed", "com.elderdrivers.riru",
        "rikka.ndk", "org.lsposed.lspd", "LSPosedService",
    )

    private val HIDDEN_LIBRARY_PATTERNS = listOf(
        "libxposed", "liblspd", "libriru", "libsandhook",
        "libpine", "libwhale", "libdobby", "libsubstrate",
    )

    private val HIDDEN_PACKAGES = listOf(
        "de.robv.android.xposed.installer", "org.lsposed.manager",
        "io.github.lsposed.manager", "com.topjohnwu.magisk",
        "me.weishu.exp", "org.meowcat.edxposed.manager",
    )
    ...
    private fun hookStackTraces() {
        ...
        runCatching {
            "java.lang.Thread".toClass().apply {
                method { name = "getStackTrace"; emptyParam() }.hook {
                    after {
                ...
                runCatching {
                    method { name = "getAllStackTraces"; emptyParam() }.hook {
                        after {
                            @Suppress("UNCHECKED_CAST")
                            val allTraces = result as? Map<Thread, Array<StackTraceElement>> ?: return@after
                            runCatching {
                                result = allTraces.mapValues { (_, stack) -> filterStackTrace(stack) }
                            }
                        }
                    }
                }
    ...
    private fun hookProcMaps() {
        "java.io.BufferedReader".toClass().apply {
            method { name = "readLine"; emptyParam() }.hook {
                after {
    ...
    private fun hookPackageManager() {
        "android.app.ApplicationPackageManager".toClass().apply {
            method { name = "getPackageInfo"; paramCount = 2 }.hook {
                before {
            ...
            method { name = "getApplicationInfo"; paramCount = 2 }.hook {
                before {
                    val pkgName = args(0).string()
                    if (HIDDEN_PACKAGES.any { pkgName.equals(it, ignoreCase = true) }) {
                        throw android.content.pm.PackageManager.NameNotFoundException(pkgName)
            ...
            runCatching {
                method { name = "getInstalledPackages"; paramCount = 1 }.hook {
                    after {
            ...
            runCatching {
                method { name = "getInstalledApplications"; paramCount = 1 }.hook {
                    after {
