...
object DeviceHooker : BaseSpoofHooker("DeviceHooker") {
    ...
    private val subscriptionInfoClass by lazy { "android.telephony.SubscriptionInfo".toClassOrNull() }

    // Fallback values (thread-safe lazy)
    private val fallbackImei by lazy(LazyThreadSafetyMode.SYNCHRONIZED) { ValueGenerators.imei() }
    private val fallbackSerial by lazy(LazyThreadSafetyMode.SYNCHRONIZED) { ValueGenerators.serial() }
    private val fallbackAndroidId by lazy(LazyThreadSafetyMode.SYNCHRONIZED) { ValueGenerators.androidId() }
    ...
    private fun hookTelephonyManager() {
        ...
        telephonyClass.apply {
            ...
            // getImei() overloads
            method { name = "getImei"; emptyParam() }.hook { replaceAny { cachedImei } }
            method { name = "getImei"; param(IntType) }.hook { replaceAny { cachedImei } }
            
            // getSubscriberId (IMSI) overloads
            method { name = "getSubscriberId"; emptyParam() }.hook { replaceAny { cachedImsi } }
            runCatching {
                method { name = "getSubscriberId"; param(IntType) }.hook { replaceAny { cachedImsi } }
            }
            
            // getSimSerialNumber (ICCID) overloads
            method { name = "getSimSerialNumber"; emptyParam() }.hook {
                after { result = cachedIccid }
            }
            runCatching {
                method { name = "getSimSerialNumber"; param(IntType) }.hook {
                    after { result = cachedIccid }
                }
            }

            // SIM Hooks
            method { name = "getSimCountryIso"; emptyParam() }.hook {
                after { result = getSpoofValue(SpoofType.SIM_COUNTRY_ISO) { "us" } }
            }
            runCatching {
                method { name = "getSimCountryIso"; param(IntType) }.hook {
                    after { result = getSpoofValue(SpoofType.SIM_COUNTRY_ISO) { "us" } }
                }
            }

            method { name = "getNetworkCountryIso"; emptyParam() }.hook {
                after { result = getSpoofValue(SpoofType.NETWORK_COUNTRY_ISO) { "us" } }
            }
            runCatching {
                method { name = "getNetworkCountryIso"; param(IntType) }.hook {
                    after { result = getSpoofValue(SpoofType.NETWORK_COUNTRY_ISO) { "us" } }
                }
            }

            method { name = "getSimOperatorName"; emptyParam() }.hook {
                after { result = getSpoofValue(SpoofType.SIM_OPERATOR_NAME) { "Carrier" } }
            }
            runCatching {
                method { name = "getSimOperatorName"; param(IntType) }.hook {
                    after { result = getSpoofValue(SpoofType.SIM_OPERATOR_NAME) { "Carrier" } }
                }
            }

            method { name = "getNetworkOperator"; emptyParam() }.hook {
                after { result = getSpoofValue(SpoofType.NETWORK_OPERATOR) { "310260" } }
            }
            runCatching {
                method { name = "getNetworkOperator"; param(IntType) }.hook {
                    after { result = getSpoofValue(SpoofType.NETWORK_OPERATOR) { "310260" } }
                }
            }

            method { name = "getSimOperator"; emptyParam() }.hook {
                after { result = getSpoofValue(SpoofType.CARRIER_MCC_MNC) { "310260" } }
            }
            runCatching {
                method { name = "getSimOperator"; param(IntType) }.hook {
                    after { result = getSpoofValue(SpoofType.CARRIER_MCC_MNC) { "310260" } }
                }
            }
        }
    ...
    private fun hookSubscriptionInfo() {
        subscriptionInfoClass?.apply {
            runCatching {
                method { name = "getCountryIso"; emptyParam() }.hook {
                    after { result = getSpoofValue(SpoofType.SIM_COUNTRY_ISO) { "us" } }
                }
            }
            runCatching {
                method { name = "getCarrierName"; emptyParam() }.hook {
                    after { result = getSpoofValue(SpoofType.CARRIER_NAME) { "Carrier" } as CharSequence }
                }
            }
            runCatching {
                method { name = "getDisplayName"; emptyParam() }.hook {
                    after { result = getSpoofValue(SpoofType.CARRIER_NAME) { "Carrier" } as CharSequence }
                }
            }
            runCatching {
                method { name = "getMcc"; emptyParam() }.hook {
                    after {
            ...
            runCatching {
                method { name = "getMnc"; emptyParam() }.hook {
                    after {
                        val mccMnc = getSpoofValue(SpoofType.CARRIER_MCC_MNC) { "310260" }
            ...
            runCatching {
                method { name = "getMccString"; emptyParam() }.hook {
                    after {
                        val mccMnc = getSpoofValue(SpoofType.CARRIER_MCC_MNC) { "310260" }
            ...
            runCatching {
                method { name = "getMncString"; emptyParam() }.hook {
                    after {
                        val mccMnc = getSpoofValue(SpoofType.CARRIER_MCC_MNC) { "310260" }
            ...
            runCatching {
                method { name = "getIccId"; emptyParam() }.hook {
                    after { result = getSpoofValue(SpoofType.ICCID) { ValueGenerators.iccid() } }
                }
            }
            runCatching {
                method { name = "getNumber"; emptyParam() }.hook {
    ...
    private fun hookBuildClass() {
        runCatching {
            "android.os.Build".toClass().apply {
                method { name = "getSerial"; modifiers { isStatic } }.hook {
                    after { result = getSpoofValue(SpoofType.SERIAL) { fallbackSerial } }
                }
            }
        }
    ...
    private fun hookSystemProperties() {
        "android.os.SystemProperties".toClassOrNull()?.apply {
            ...
            method { name = "get"; param(StringClass) }.hook {
                after {
            ...
            method { name = "get"; param(StringClass, StringClass) }.hook {
                after {
                    val key = args(0).string()
                    if (key in serialKeys) {
                        result = getSpoofValue(SpoofType.SERIAL) { fallbackSerial }
