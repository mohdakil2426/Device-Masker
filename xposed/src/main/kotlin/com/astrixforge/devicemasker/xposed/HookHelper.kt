...
fun isSdkInRange(min: Int, max: Int): Boolean = 
    Build.VERSION.SDK_INT in min..max

/**
 * Hooks only if the SDK version matches.
 */
inline fun PackageParam.hookIfSdk(
    minSdk: Int,
    maxSdk: Int? = null,
    block: () -> Unit
): Boolean {
 ...
 */
inline fun PackageParam.hookLegacyApi(
    tag: String = "LegacyHook",
    block: () -> Unit
) {
 ...
 */
inline fun PackageParam.hookModernApi(
    tag: String = "ModernHook",
    block: () -> Unit
) {
