 ...
 *
 * Configuration is stored in /data/misc/devicemasker/ which is accessible by system_server
 * without requiring additional SELinux policy modifications.
 *
 * Features:
...
object ConfigManager {
     ...
     *
     * On first run, creates the config directory and saves a default config.
     * If the config file is corrupted, attempts to restore from backup.
     *
        ...
        return runCatching {
            ...
            if (CONFIG_FILE.exists()) {
                runCatching {
                    ...
                    DualLog.warn(TAG, "Failed to create backup: ${it.message}")
                }
            }
    ...
    fun deleteConfig() {
        runCatching {
            ...
            DualLog.error(TAG, "Failed to delete config: ${it.message}")
        }
    }
