package com.akil.privacyshield.data.models
 ...
 * Each profile contains values for all supported spoof types and can be
 * assigned to specific apps or set as the default.
 *
...
data class SpoofProfile(
    ...
    companion object {
        ...
        fun createNew(name: String, isDefault: Boolean = false): SpoofProfile {
            val defaultIdentifiers = SpoofType.entries.associateWith { type ->
        ...
        fun createForManufacturer(name: String, manufacturer: String): SpoofProfile {
            ...
            val buildProps = com.akil.privacyshield.data.generators.FingerprintGenerator
            ...
            buildProps["MODEL"]?.let {
                result = result.withValue(SpoofType.BUILD_MODEL, it)
            }
            buildProps["MANUFACTURER"]?.let {
                result = result.withValue(SpoofType.BUILD_MANUFACTURER, it)
            }
            buildProps["BRAND"]?.let {
                result = result.withValue(SpoofType.BUILD_BRAND, it)
            }
            buildProps["DEVICE"]?.let {
                result = result.withValue(SpoofType.BUILD_DEVICE, it)
            }
            buildProps["PRODUCT"]?.let {
                result = result.withValue(SpoofType.BUILD_PRODUCT, it)
            }
